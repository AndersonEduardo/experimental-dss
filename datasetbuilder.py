...
# pytesseract setup (apenas para Windows) - See PYTESSERACT_PATH in parameters.py
...
class DatasetBuilder:
    ...
    def __init__(self, folder:str = None):
        ...
        self.folder = PDF_PATH if folder is None else folder
        self.dpi = 600
    ...
    def df_sieve(self, df_list, method):
        ...
        for df_object in df_list:
            ...
            if method.strip().lower() == 'camelot':
    
                df = df_object.df
                df.columns = df.loc[0]
                df = df.drop(0)

            elif method.strip().lower() == 'tabula':
    ...
    def run(self, file_name:list=None, method:str='camelot', verbose:int=1):
        ...
        for file_path in file_paths:
            ...
            if self.check_verbose(verbose) and verbose == 1:
                # print(' done!')
                print(f'[STATUS] ...found {number_of_tables} tables and...')
                print(f'[STATUS] ...{len(variants)} variants in "{key}".')

        return dataset


    def consolidate(self, dataset_dict:dict) -> pd.DataFrame:
